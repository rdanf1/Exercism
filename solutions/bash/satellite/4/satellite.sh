...
#     
#     - between (r+1) and (llv) are main left subtree values (MLSv)
#        => Card left subtree = nb of elem including (r+1)..(llv)
#
...
then
        ...
        # testing if yq is present up there
        #
        result_JSON="$(yq -oj <<< "$result_JSON")"
...
# Some tests..
#yq dan.yaml | sed -r 's/([[:space:]]*)([[:alnum:]])/\1  \2/' | sed -E 's/([[:alnum:]]*):/"\1":/' | sed -E 's/: ([[:alnum:]]+)/: "\1"/'

#exit

# Same parameters as jq though..
#
jqq () {
  shift
...
mk_Sat () {
  ...
  if [[ $(which jq) ]]
  then
   result_JSON="$(jq --arg  root "$root" \
   'add(.v=$root) | add(.l={})   | add(.r={}) ' <<<'{}')"
  else
   echo "$(ls /usr/bin/j*) : " >> "./$0"
   echo "Any jq likewise utility there ?" >> "./$0"
  fi
...
# DR - Ascension 2025 plus: 167 days...
