...
#    and a heron's bc script for nth_root func. (rpow case..)
#          => from wiki's "nth-root".
#
#################
## Novelty !!! ##
#########################################################
#
#  Time exceeded ( for nth_root func in rpow ) :
#
#        Jobs management ( use of '&' and 'wait' )
#        Traps  
#             - On time out (Cf nth_root)
#             - Some Interrupt signals
#                 => Clean tmp files
#        Special opti@ns :
#             - Full display (*all* decimals for rpow results)
#             - 4th arg in rpow is nb Iterations for start
#            ( if no 4th undocumented arg given we start from 1 )
#
#########################################################
# Todo :
#           - Very large inputs => overflows     <<100>>
#             resulting outputs with very wrong coherence
#             => to be noticed as inaccurate / wrong !!!
#
...
#
# Hi, seems there is 
#   no TMPDIR var defined there..
#   ( and /tmp isnt available )
#
TmpDir="${TMPDIR:=.}"
...
cleaning...; \
echo -n "last result: " ; cat "$TmpDir/Out.tmp" 2>/dev/null ; \
...
#
nth_sqrt () {
trap 'date; echo "Compute Time exhausted.."; \
      sync ; printf "%s\n" "best result : " ; \
      cat "$TmpDir/Out2.tmp" 2>/dev/null || echo "None prev.."; \
      rm "$TmpDir/Out2.tmp" "$TmpDir/Out.tmp" \
         "$TmpDir/root.bc" 2>/dev/null ; \
      exit 11' USR1
     ...
     # Timeout metrics
     #
     # nb : if timeout trapped, displays last interval
     #      between last succeded Iteration and the failed one
     #      ( => substract this interval to MaxTime and 
     #           Iter₋₁ will still ends without timeout )
     #
     printf "date before : %s\n" "$(date)" >> "$TmpDir/Out2.tmp" 

     results=$(echo "$1" $'\n' "$2" $'\n' "$3" \
	     | bc -ql "$TmpDir/root.bc")

     # Full result storage for trap display (timeout..)
     #
     printf "%s\n" "$results" | tail -n 8 \
	                      | sed 's/.*nth_root: //' \
	    > "$TmpDir/Out2.tmp"
...
#			| sed 's/^\./0\./')
  ...
  || echo  $results	| sed 's/.*nth_root: //' \
			...
			# for full result with 25 decimals
			#| sed -E 's/(\.[0-9]{2,25}).*/\1/' \
			#| sed -E 's/\.[0]{2,25}$/\.0/' \
test_zone () {
...
######
#
# NB1 : With MaxTime=5s and those inputs :
#
#     with a=(3^19)=1162261467 and with n=598
# and with hidden 4th option = iteration number = 8 or  7 !
#  => 7 iter. should be shorter though actually it is not !
#
# Practically : 
#   ./rational_numbers.sh rpow "3" "19/598" 8 => Result ok
#   ./rational_numbers.sh rpow "3" "19/598" 7 => Timeout 
#   		( but coherent : (nth_root^n)/a ~ 100.00017..)
#
#   The problem is by default we increase iterations if 
#       coherence isnt alike 99.999999 or 100.000000.
#
# NB2 : Setting MaxTime=6
#   
#     1. Makes 
#   ./rational_numbers.sh rpow "3" "19/598" 7 => No Timeout 
#   succeed, because iter7 + iter8 durations < 6 sec.
#
#     2. Makes 
#   ./rational_numbers.sh rpow "3" "19/598" => Timeout !!
#   failed, because iter1 + .. + iter8 durations > 6 sec.
#
#     3. Actually
#   ./rational_numbers.sh rpow "3" "19/598" => No Timeout !!
#   Needs : MaxTime=10 !!!
#    
# Conclusion : NB2 couldnt explain NB1 behavior as *Normal* !
#              But around a second of margin we may have :
#
#                 Compute[Iter₂] < Compute[Iter₁]
#
######
#
#
...
mk_rpow () {
	...
	# done in main ()
	#cat "$TmpDir/Out.tmp" 2>/dev/null && exit 0
}
...
Clean_Tmp_Files () {
    ...
    # nb: cant clean Out2.tmp.. 
    # (delayed full output on timeout depends on it !)
    #
    #rm "$TmpDir/Out2.tmp" 2>/dev/null
}
...
main () {
    ...
    rm "$TmpDir/Out.tmp" "$TmpDir/Out2.tmp" 2>/dev/null
