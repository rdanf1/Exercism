...
Maximize () {
	...
	#   ( we need Prod because sometimes 
	...
	# Found better ?!..
	# Not sufficient condition ! => commented
	#[[ "$Sum" -ge "$Max_Value" ]] && \
	[[ "$Prod" -gt "$Max_Prod" ]] && \
        Max_Prod=$Prod && \
        Max_Value=$Sum && \
	Max=$1 && \
	[[ $DEBUG -gt 0 ]] && display "$1"
...
# Computed before for optimum evaluation [Mandatory!]
#  not requied and time consuming (though..)
#	       [[ ! $DEBUG -gt 0 ]] && \
#	       Prod=$((Prod *= Val))
   	   done
    	       echo2 
