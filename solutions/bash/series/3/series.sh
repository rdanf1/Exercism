...
Display_Serie () {
	...
	# Trivial 1st ! (though comes last) 
	#  nb : not going a space too far..
	#
	#  ( nb2 : using array notation with a string
	...
	# 1 space v here ! 
	#         |
	printf "%s " "$(echo "$Sequence" \
		| cut -c 1-$2)"

	Sequence="$(echo -n "$Sequence" | cut -c 2-)"
...
main () {
    ...
    echo2 "Span_Length :" "$Span_Length" 

    Display_Serie "$Input" "$Span_Length" 

    # For aesthetism reason ( should be in
    #                         last printf with \n )
    #  nb : not required by bats tests
    #
    echo
