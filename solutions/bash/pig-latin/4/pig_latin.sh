...
#
#		=> was much more fun than expected to solve this !
#
#
# (**) =>> added these specifically for last @test ( generic '*' )
#    nb0 : added ",\*" to regex patterns too !
#      =>> I really dont think that this last case
#          appears to be realistic though.
#
...
# [^.. means Not following chars..
# removed ..,xr,yt] => painlessly
#    => actually it works only without those 2 pairs
#
RULE2=\
'^[^a,e,i,o,u][a-z,\*]*$'
RULE3=\
...
# Perhaps a single function (with a N⁰ rule arg.)
#  would be sufficent here and avoidness of "$?" ?
#
is_rule1 () {
	echo "$1" | grep -q "$RULE1"
	return "$?" 
			...
			printf %s \
			    ...
			    printf %s \
			       ...
			       | sed -E 's/(.*)y(.*)/y\2\1ay/')"
		            ...
		            do
		                printf2 "rule2 :" "$i"
				# move 1st char in last pos.
				#
			        i="${i:1}""${i:0:1}"
		            done
			    # assuming it was rule2 !
			    #  ( only last possible case ?! )
			    #
			    printf '%say' "$i"
		        fi
		    fi
