...
main () {
	...
	do
	 ...
	 && Codes+="${Codes_array[$i]}"
	    [[ $j -eq 3 ]] \
	 && nb_zeros="${Codes_array[$i]}"

	    echo2 "j / Codes : " \
		  "$j" " " "$Codes"
	done
	
	# Black is black (special values!)
	#
	[ "$Codes" == "00" ] \
     && echo "0 ohms" \
     && exit 0
	...
	# More compact than a case
	#  ( append mode for a string )
	#
	while [[ "$nb_zeros_2" -ge 1 ]] 
	do
		((nb_zeros_2--))
		zeros+="0" 
	done
	...
	# 3 possibilities (except default)
	#
	if [[ "nb_zeroes" -ge 9 ]]
	then
