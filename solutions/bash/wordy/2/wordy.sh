...
'-?[[:digit:]]+'
REGEX_PARAM2=\
"^What is $REGEX_PARAM1(( plus| minus| multiplied by| divided by) $REGEX_PARAM1)*\?$" 
REGEX_PARAM_WRDS=\
"What is( plus| minus| multiplied by| divided by | [[:digit:]])* *\?$"
...
echo " Usage : $0 [<<< $'Ansi-C string' or \"\$str\"]"$'\n'\
      "     or $0 [file]"$'\n'\
      "with    $0 What is <nb1> <op1> <nb2> <op2>..."$'\n'\
      "with    <op> is <plus/minus/multiplied by/divided by>"
      return 1
  else
...
mk_Bcs () {
  ...
  do
    case "$i" in
      -[0-9]*|[0-9]*)
        # bc is arithmetically compliant 
        #  (** first, then * or / , then + or -)
        #  so "first in first out"
        #  is not the same policy..
        #  using parenthesis to do so.
        #
        if [[ "$sign" -gt 0 ]]
        then
          Result+="${i%\?})"
    ...
    printf2 "Result : " "$Result"
    bc<<<"$Result" 2>/dev/null || echo "bc error"
}
