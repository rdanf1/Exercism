...
list::foldr () {
  ...
  && [[ -n "${list_dl[2]}" ]]
  then
    list_dl="$("$func_op" "${list_dl[2]}" "${list_dl[1]}")"
  fi
...
list::foldl () {
  ...
  #  list::foldr "$funct2" "$funct_param2" result5
  #else
    list::foldr "$funct2" "$funct_param2" list6 
  #fi
