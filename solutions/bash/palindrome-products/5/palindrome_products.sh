...
#  ( might miss some solution(s) though @tests are ok.. )
          End="$(($2/2 + $2/3 ))"
          # Uncomment To Check exhaustivity..
          #End="$2"
      else
...
$'\n'
  ...
  Find_pair2 () {
      ...
      # All pair digits palindromes are 11 multiples !
      #  ( might miss some solution(s) though @tests are ok.. )
      End="$(($2 / (11*2) ))"
      # Uncomment To Check exhaustivity of solution(s)..
      # End="$2"
...
case "$min_dig" in
  ...
  10 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do for h in {0..9} ; do Find_pair2 "$1" "$2" "${l}${k}${j}${i}${h}${h}${h}${i}${j}${k}${l}" "D" ; done ; done ; done ; done ; done
      ;;
  9 )
