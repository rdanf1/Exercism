...
#    => Wrong assertion fix : Y'∈ [a/11;b   ]
#                             and [a   ;b/11] maybe correct
#                             but [<-swch-> ] maybe necessary
#    => Reimplement Find_pair2 () ?
#       
# NB2 : Halas no such a propriety (as NB1) exists for (P₁) :
#          palindomes with odds nb of digits
#
# REM : For series (single=0) the efficience/rapidity when 
#       searching 7, 8, 9 or 10 (max!) digits palindromes
#       is much better when choosing a range [$1-$2]
...
# Faster (but not so much..) than above '_pair'
#   for palindromes with pair nb of digits
#
Find_pair2 () {
    ...
    [[ "$x" -gt "$biggest" ]] \
      ...
      for z in $(seq "$((Start/11))" "$decr" "$((End/11))")
      do
...
$'\n'
  ...
  # Slightly faster than '_Big' (and same results
  #                             if single=0 )
  #
  srch_Small () {
    ...
    if [[ "$min_dig" == "$max_dig" ]]
    then
      l_vals="$(seq ${smallest:0:1} ${biggest:0:1} )"
    else
      # Greatest scope at size of smallest digits !
      l_vals="$(seq ${smallest:0:1} 9 )"
    fi
    k_vals=$(echo {0..9})
    j_vals=$(echo {0..9})
    i_vals=$(echo {0..9})
...
case "$min_dig" in
  ...
  10 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do for h in {0..9} ; do Find_pair2 "$1" "$2" "${l}${k}${j}${i}${h}${h}${i}${j}${k}${l}" "D" ; done ; done ; done ; done ; done
      ;;
  9 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do for h in {0..9} ; do Find_pair "$1" "$2" "${l}${k}${j}${i}${h}${i}${j}${k}${l}" "D" ; done ; done ; done ; done ; done
       ;;
  8 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do Find_pair2 "$1" "$2" "${l}${k}${j}${i}${i}${j}${k}${l}" "D" ; done ; done ; done ; done
       ;;
  7 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do Find_pair "$1" "$2" "${l}${k}${j}${i}${j}${k}${l}" "D" ; done ; done ; done ; done 
       ;;
  6 )
for l in  $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do Find_pair2 "$1" "$2" "${l}${k}${j}${j}${k}${l}" "D" ; done ; done ; done 
       ;;
  5 )
...
for l in $(seq 9) ; do for k in $(seq 0 9) ; do Find_pair "$1" "$2" "${l}${l}" "D" ; done ; done 
  ...
  * )
  ...
  # Slightly slower than '_Small' (and same results
  #                                if single=0 )
  #
  srch_Big () {
    ...
    if [[ "$min_dig" == "$max_dig" ]]
    then
      l_vals="$(seq ${biggest:0:1} -1 ${smallest:0:1} )"
    else
      # Greatest scope at size of biggest digits !
...
case "$max_dig" in
  ...
  10 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do for h in {9..0} ; do Find_pair2 "$1" "$2" "${l}${k}${j}${i}${h}${h}${i}${j}${k}${l}" "I" ; done ; done ; done ; done ; done
      ;;
  9 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do for h in {9..0} ; do Find_pair "$1" "$2" "${l}${k}${j}${i}${h}${i}${j}${k}${l}" "I" ; done ; done ; done ; done ; done
      ;;
  8 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals  ; do for i in $i_vals ; do Find_pair2 "$1" "$2" "${l}${k}${j}${i}${i}${j}${k}${l}" "I" ; done ; done ; done ; done
      ;;
  7 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do for i in $i_vals ; do Find_pair "$1" "$2" "${l}${k}${j}${i}${j}${k}${l}" "I" ; done ; done ; done ; done
       ;;
  6 )
for l in $l_vals ; do for k in $k_vals ; do for j in $j_vals ; do Find_pair2 "$1" "$2" "${l}${k}${j}${j}${k}${l}" "I" ; done ; done ; done
       ;;
  5 )
