...
Main () {
  ...
  declare -i recurent=1 result1=0 N="$1" sum=0
...
#recure_S_Pn 3 1
  ...
  printf2 "Sn2 :" "$Sn2" 
  printf2 "S_Pn[1]/Pn[1] :" "${S_Pn[1]}${Pn[1]}"
...
#
# (?) : Need verification.. and Implies (s+t) pair on 
# (!) : Obvious !
# =====================================================
#     ^^^^^^^^^^^^^^ This is raw !!! ^^^^^^^^^^^^^^
#
...
get_Factors () {
  ...
  Two_s=1
     ...
     printf2 "FactFact_R[@] :" "${FactFact_R[@]}" 
     printf2 "FactFact_R[0] :" "${FactFact_R[0]}" 
     printf2 "#FactFact_R[@] :" "${#FactFact_R[@]}" 
     #[[ "${Fact_R[0]}" =~ '^' ]] && \
     if \
     [[ "${#Fact_R[@]}" -lt 3 ]]  && \
