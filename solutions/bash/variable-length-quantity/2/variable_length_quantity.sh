...
mk_VQL () {
  ...
  case "$1" in
    ...
    do
      ...
      then
        diff=$((7 - mod_7enc))
        padding_zeros=$(printf "%0${diff}i") # | tr ' ' '0')
        mod_7ind=$((mod_7enc + 1))
...
"$(printf '%02s' "$(bc <<<"obase=16;ibase=2;$i")")"" "
 ...
 #                   sed 's/ 0 / 00 /g' | \
      result_hex="$(echo "$result_hex" | \
                    sed 's/ $//')"  # remove to separate
                                    # multipe values..
    ...
    'decode')
      ...
      do
        ...
        then
          ...
          #   different values of VQL numbers
...
# DR - Ascension 2025 plus: 161 days...
